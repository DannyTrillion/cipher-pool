...
**Description (short)**
Cipher Pool is a no-loss prize savings pool built on Zama FHEVM, a confidential version of PoolTogether. Savers wrap Zama's official test USDT into confidential cUSDT and deposit it into a shared pool. Every five minutes the pool's yield is awarded as five prizes across three tiers. Winners are picked on chain with FHE randomness, weighted by deposit size and computed over encrypted balances, so nothing is ever decrypted to run a draw. Every saver receives an encrypted result, winners and non-winners alike, so from the outside every account looks identical. A winner decrypts their result with one EIP-712 signature and collects by confidential transfer. Principal can be withdrawn whenever the pool is open. Prizes, seeds and saver counts are public so anyone can verify a draw was honest; who won and how much anyone holds never is.
