...
|---|---|---|
| **Get test tokens** | `tUSD` is a plain 6-decimal test ERC-20 with a public faucet (1,000 per hour per address). | `MockUSD.faucet()` — "Need test tUSD?" in the app |
| **Shield** | `tUSD.approve(cUSD, amount)` → `cUSD.wrap(you, amount)`. `cUSD` is an OpenZeppelin **ERC-7984 wrapper** over `tUSD`; your cUSD balance is an encrypted `euint64`, backed 1:1 by tUSD locked in the wrapper. | Shield tab / wizard step 3 |
...
`MockYieldSource` is an **admin-funded prize drip**: a public fixed rate of tUSD per second (`ratePerSecond`, default 2,777 units ≈ 10 cUSD/hour) is minted, wrapped into cUSD and handed to the pool's encrypted prize reserve on every `harvest()` (called automatically at draw start; callable by anyone). It does *not* compute APY on the encrypted principal, because a plaintext mock cannot do that without leaking TVL. Every dripped cUSD is backed by tUSD held in the wrapper.
...
| ConfidentialUSD (cUSD, ERC-7984 wrapper) | [`0xAb953cAC2DF2Fd46F8296cff6eE420f73733410F`](https://sepolia.etherscan.io/address/0xAb953cAC2DF2Fd46F8296cff6eE420f73733410F) |
| MockUSD (tUSD, test ERC-20) | [`0x4324b6425E1714C6a7cE5CDAEBf32aBa18AB6d0E`](https://sepolia.etherscan.io/address/0x4324b6425E1714C6a7cE5CDAEBf32aBa18AB6d0E) |
...
Draw period 10 minutes · prize drip ≈ 10 cUSD/hour · faucet 1,000 tUSD per hour per address.
...
contracts/
  ...
  contracts/ConfidentialUSD.sol         ERC-7984 wrapper over tUSD (OpenZeppelin ERC7984ERC20Wrapper)
...
- **Unsupported tokens**: the pool is single-asset (cUSD over tUSD) by design; there is no token picker to misuse.
